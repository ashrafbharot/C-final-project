...
Full Name  : Sunny Anilkumar Vavadiya
Student ID#: 126691229
Email      : savavadiya@myseneca.ca
Section    : ZEE
...
void editPatient(struct Patient patient[], int max)
{
    ...
    // To check if patient number exists or not
...
// Todo:
void addAppointment(struct Appointment* app, int maxAppointments, struct Patient* pat, int maxPatients)
{
    ...
    index = findPatientIndexByPatientNum(patientNumber, pat, maxPatients);
...
// Todo:
void removeAppointment(struct Appointment* app, int maxAppointments, struct Patient* pat, int maxPatients)
{
    struct Date date;
    ...
    if (index >= 0)
    {
        ...
        if (aptIndex >= 0)
        {
            ...
            displayPatientData(&pat[index], FMT_FORM);
...
int findPatientIndexByPatientNum(int patientNumber, const struct Patient patient[], int max)
{
    ...
    // Returns -1 if anything not found
    return -1;
}

// Cto check whether a time slot is available
...
//  to cehck for the next slot that is available and return the index
...
//  to check if an appointment is valid
...
int importAppointments(const char* datafile, struct Appointment appointment[], int max)
{
    ...
    if (fp != NULL)
    {
        while (fscanf(fp, "%d,%d,%d,%d,%d,%d", &appointment[read].patientNumber, &appointment[read].date.year, &appointment[read].date.month, &appointment[read].date.day, &appointment[read].time.hour, &appointment[read].time.min) == 6 && read < max)
        {
            read++;
        }
        fclose(fp);
